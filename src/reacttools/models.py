...
class ReactAppSettings(models.Model):
    ...
    static_path_prefix = models.CharField(
        max_length=128,
        blank=True,
        help_text="Tells collectreact wether or not to insert an extra directory into the static file path (i.e. js/prefix/main.chunk.js).  Blank is no prefix.",
    )
    ...
    def css_paths(self):
        if self.static_path_prefix:
            return [
                static("css/%s/%s" % (self.static_path_prefix, s)) for s in self.css
            ]
